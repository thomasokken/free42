...
static int parse_double(char *p, int len, double *res) {
    ...
    if (in_exp ? exp_empty : mant_empty)
	return 0;
...
static void do_clip_paste() {
	...
	goto attempt_4;
    finish_complex:
    if (!parse_double(buf + s1, e1 - s1, &re))
	goto attempt_4;
    if (!parse_double(buf + s2, e2 - s2, &im))
	goto attempt_4;
	...
	i++;
    e1 = i;
    if (e1 != s1 && parse_double(buf + s1, e1 - s1, &re))
	core_paste_real(re);
    else {
	if (len > 6)
	    len = 6;
